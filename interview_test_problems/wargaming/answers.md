...
7. Я постарался оптимизировать лишнее O(n) копирование после обработки каждого порядка и копирую только если кол-во свапов было нечетным. Можно было бы так же ввести параметры int max, bool given_max и если given_max == true то можно было бы избежать линейного поиска максимума, но это не вписывается в контекст общего случая. 
